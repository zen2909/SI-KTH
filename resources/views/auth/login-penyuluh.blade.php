...
@section('content')
    <div class="w-full max-w-[1100px] flex flex-col md:flex-row bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div class="hidden md:flex w-5/12 bg-primary flex-col justify-between p-8 text-white">
            ...
            <div>
                <blockquote class="text-lg italic text-white leading-relaxed">
                    "Input data KTH binaan dengan tepat untuk mendukung program kehutanan yang berkelanjutan."
                </blockquote>
            </div>
        </div>
        ...
        <div class="w-full md:w-7/12 p-6 md:p-12 flex flex-col justify-center">
            <div class="mb-6">
                <div
                    class="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg border border-emerald-600">
                    <span class="icon-[mdi--people-outline] w-4 h-4 text-emerald-600"></span>
                    <span class="text-xs font-semibold">AKSES PENYULUH</span>
                </div>
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mt-2">Selamat Datang Penyuluh</h2>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="penyuluh@kth.go.id" required autofocus>
                    @error('email')
