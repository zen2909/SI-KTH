...
<dialog id="modalHapusKTH"
    ...
    <div class="bg-white rounded-2xl flex flex-col max-h-[85vh] relative">
        ...
        <div class="mx-3.5 mt-3 p-3 bg-zinc-100 rounded-xl border border-stone-300">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-zinc-900" id="targetNamaKTH">
        ...
        <div class="flex items-center justify-center gap-2.5 px-4 py-5">
            ...
            <form id="formHapusKTH" method="POST" action="">
                @csrf
                @method('DELETE')
                <button type="submit"
...
<script>
    ...
    // ============================================
    window.openHapusKTHModal = function(kthId, namaKth) {
        const modal = document.getElementById('modalHapusKTH');
        const targetElement = document.getElementById('targetNamaKTH');
        const form = document.getElementById('formHapusKTH');
        ...
        if (form) {
            const url = "{{ route('penyuluh.kth.destroy', ['kth' => ':id']) }}".replace(':id', kthId);
            form.action = url;
        }
