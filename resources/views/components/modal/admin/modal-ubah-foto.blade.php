...
<dialog id="modalUbahFotoAdmin"
    ...
    <div class="bg-white rounded-3xl flex flex-col">
        ...
        <div class="flex items-center justify-between bg-white py-6 px-8 border-b border-gray-100">
            ...
            <button type="button" onclick="closeModalUbahFotoAdmin()"
        ...
        <div class="flex flex-col items-center px-8 py-6">
            ...
            <div class="relative mb-6">
                <div
                    ...
                    @if ($user && $user->foto_profil)
                        <img src="{{ asset('storage/' . $user->foto_profil) }}" alt="Foto Profil" id="previewFotoAdmin"
                            class="w-full h-full object-cover">
                    @else
                        <span class="text-6xl text-gray-400 font-bold" id="previewPlaceholderAdmin">
                            {{ $user ? strtoupper(substr($user->name, 0, 2)) : '??' }}
                        </span>
                        <img src="" alt="Preview" id="previewFotoAdmin"
            ...
            <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data"
                id="formUbahFotoAdmin">
                @csrf
                @method('PUT')

                <input type="file" name="foto_profil" id="fileInputAdmin" accept="image/*" class="hidden"
                    onchange="previewFotoAdmin(this)">
        ...
        <div class="flex justify-end items-center bg-[#F3F4F5] py-4 px-8 rounded-b-3xl gap-3">
            ...
            <button type="submit" form="formUbahFotoAdmin"
...
<script>
    ...
    function previewFotoAdmin(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                const preview = document.getElementById('previewFotoAdmin');
                const placeholder = document.getElementById('previewPlaceholderAdmin');
                if (preview) {
    ...
    window.openUbahFotoAdminModal = function() {
        const modal = document.getElementById('modalUbahFotoAdmin');
        if (modal) {
            modal.showModal();
    ...
    function closeModalUbahFotoAdmin() {
        const modal = document.getElementById('modalUbahFotoAdmin');
        if (modal) {
            modal.close();
