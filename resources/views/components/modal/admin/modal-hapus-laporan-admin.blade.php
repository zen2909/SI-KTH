{{-- Modal Hapus Laporan Admin --}}
<dialog id="modalHapusLaporanAdmin"
    ...
    <div class="bg-white rounded-2xl flex flex-col max-h-[85vh] relative">
        {{-- Tombol Close --}}
        <button type="button" onclick="closeHapusLaporanAdminModal()"
        ...
        <div class="mx-3.5 mt-3 p-3 bg-zinc-100 rounded-xl border border-stone-300">
            <div class="grid grid-cols-2 gap-1">
                <div>
                    ...
                    <p class="text-xs text-zinc-900 font-semibold" id="hapusPeriodeAdmin">-</p>
                </div>
                <div>
                    <p class="text-[10px] text-neutral-500 font-medium">Nama KTH</p>
                    <p class="text-xs text-zinc-900 font-semibold" id="hapusNamaKTHAdmin">-</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col items-center gap-2.5 px-4 py-5">
            <form id="formHapusLaporanAdmin" method="POST" action="">
                @csrf
                @method('DELETE')
                <button type="submit"
...
<script>
    // ============================================
    // FUNGSI BUKA MODAL HAPUS LAPORAN ADMIN
    // ============================================
    function openHapusLaporanAdminModal(laporanId, periode, namaKth) {
        const modal = document.getElementById('modalHapusLaporanAdmin');
        const periodeEl = document.getElementById('hapusPeriodeAdmin');
        const namaKthEl = document.getElementById('hapusNamaKTHAdmin');
        const form = document.getElementById('formHapusLaporanAdmin');

        if (periodeEl) periodeEl.textContent = periode || '-';
        if (namaKthEl) namaKthEl.textContent = namaKth || '-';

        if (form) {
            const url = "{{ route('laporan.destroy', ['id' => ':id']) }}".replace(':id', laporanId);
            form.action = url;
        }
    ...
    // ============================================
    // FUNGSI TUTUP MODAL HAPUS LAPORAN ADMIN
    // ============================================
    function closeHapusLaporanAdminModal() {
        const modal = document.getElementById('modalHapusLaporanAdmin');
        if (modal) {
            modal.close();
