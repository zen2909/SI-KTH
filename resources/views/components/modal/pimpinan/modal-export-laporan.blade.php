...
<dialog id="modalExportLaporanPimpinan"
    ...
    <div class="bg-white rounded-3xl flex flex-col max-h-[90vh]">
        ...
        <div
            ...
            <button type="button" onclick="closeModalExportLaporanPimpinan()"
        ...
        <div class="overflow-y-auto flex-1 px-6 py-4 max-h-[600px]">
            <form id="formExportLaporanPimpinan" action="{{ route('pimpinan.laporan.export') }}" method="POST"
                ...
                <div>
                    ...
                    <div class="relative mt-1">
                        <input type="text" id="searchLaporanInputPimpinan" name="search"
                ...
                <div>
                    ...
                    <select name="periode" id="periodeSelectPimpinan"
                    ...
                    <div class="flex flex-wrap gap-2 mt-1">
                        <label
                            ...
                            <input type="checkbox" name="status_verifikasi[]" value="pending"
                                class="status-checkbox-laporan-pimpinan w-3.5 h-3.5 text-emerald-900 rounded focus:ring-emerald-900">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mt-1">
                        <label id="excelOptionLaporanPimpinan"
                        ...
                        <label id="pdfOptionLaporanPimpinan"
                ...
                <div class="flex items-center gap-3 p-3 bg-green-800/10 rounded-xl border border-green-800/20">
                    ...
                    <div>
                        ...
                        <p class="text-green-900 text-sm font-normal font-['Inter']" id="totalLaporanDisplayPimpinan">
                            Total <span class="font-bold"
                                id="totalLaporanCountPimpinan">{{ number_format($totalData) }}</span> data laporan
                        </p>
                        <p class="text-xs text-neutral-500 mt-0.5" id="filterLaporanStatusPimpinan">Semua status</p>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                <button type="button" onclick="resetFilterLaporanPimpinan()"
                ...
                <button type="submit" form="formExportLaporanPimpinan"
...
<script>
    // ============================================
    // AUTO COMPLETE SEARCH UNTUK LAPORAN PIMPINAN
    // ============================================
    (function() {
        var searchInput = document.getElementById('searchLaporanInputPimpinan');
        var searchResult = document.getElementById('searchLaporanResultPimpinan');
        var searchResultList = document.getElementById('searchLaporanResultListPimpinan');
        var typingTimer;
        ...
        searchInput.addEventListener('input', function() {
            ...
            typingTimer = setTimeout(function() {
                var url = '/pimpinan/kth/search?search=' + encodeURIComponent(query);
                            ...
                            data.forEach(function(item) {
                                ...
                                html += `
                                    <div class="px-4 py-2 hover:bg-gray-100 cursor-pointer border-b border-stone-100 last:border-0"
                                        onclick="selectLaporanSearchResultPimpinan('${item.nama_kth}')">
    ...
    function selectLaporanSearchResultPimpinan(name) {
        var searchInput = document.getElementById('searchLaporanInputPimpinan');
        var searchResult = document.getElementById('searchLaporanResultPimpinan');
        if (searchInput) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var form = document.getElementById('formExportLaporanPimpinan');
        if (!form) return;
    ...
    function updateTotalLaporanPimpinan() {
        var form = document.getElementById('formExportLaporanPimpinan');
        if (!form) return;
        ...
        var url = '/pimpinan/laporan/export-count?' + searchParams.toString();
        console.log('Fetching URL:', url);

        var countDisplay = document.getElementById('totalLaporanCountPimpinan');
        var statusDisplay = document.getElementById('filterLaporanStatusPimpinan');
        if (countDisplay) {
        ...
        fetch(url, {
            ...
            .then(function(data) {
                ...
                if (statusDisplay && data.success !== false) {
                    var statuses = [];
                    var checkboxes = document.querySelectorAll('.status-checkbox-laporan-pimpinan:checked');
                    checkboxes.forEach(function(cb) {
                        statuses.push(cb.value.charAt(0).toUpperCase() + cb.value.slice(1));
                    });

                    var periode = document.getElementById('periodeSelectPimpinan').value;
                    var search = document.getElementById('searchLaporanInputPimpinan').value;
    ...
    // FUNGSI TUTUP MODAL
    // ============================================
    window.closeModalExportLaporanPimpinan = function() {
        var modal = document.getElementById('modalExportLaporanPimpinan');
        if (modal) {
            modal.close();
    ...
    // ============================================
    // FORMAT FILE TOGGLE LAPORAN PIMPINAN
    // ============================================
    document.addEventListener('DOMContentLoaded', function() {
        var excelOption = document.getElementById('excelOptionLaporanPimpinan');
        var pdfOption = document.getElementById('pdfOptionLaporanPimpinan');
    ...
    // ============================================
    // FUNGSI RESET FILTER LAPORAN PIMPINAN
    // ============================================
    window.resetFilterLaporanPimpinan = function() {
        var form = document.getElementById('formExportLaporanPimpinan');
        if (form) {
            form.reset();
            ...
            document.getElementById('searchLaporanInputPimpinan').value = '';
            document.getElementById('searchLaporanResultPimpinan').classList.add('hidden');
            updateTotalLaporanPimpinan();
        }
    };
